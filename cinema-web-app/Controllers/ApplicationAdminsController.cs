...
public class ApplicationAdminsController : Controller
{
    ...
    public ApplicationAdminsController(UserManager<ApplicationUser> userManager,
    ...
    public async Task<IActionResult> UsersByRole(string role = "Customer")
    {
        ...
        var showCinemaColumn = role == "ContentCinemaAdmin"; // Check if the role is ContentCinemaAdmin
    ...
    public async Task<IActionResult> Edit(Guid id)
    {
        ...
        var isContentCinemaAdmin = contentCinemaAdmin != null;
    ...
    [Authorize(Roles = "ApplicationAdmin")]
    public async Task<IActionResult> Edit(Guid id,
        [Bind("Id,FirstName,LastName,Email,CinemaId")] EditUserViewModel model)
    {
        ...
        if (ModelState.IsValid)
        {
            ...
            if (result.Succeeded)
            {
                ...
                {
                    var contentCinemaAdmin =
                        await _context.ContentCinemaAdmins.FirstOrDefaultAsync(ca => ca.UserId == user.Id);
    ...
    public async Task<IActionResult> DeleteConfirmed(Guid id, string role)
    {
        ...
        if (user != null)
        {
            ...
            if (result.Succeeded) return RedirectToAction(nameof(UsersByRole), new { role });
